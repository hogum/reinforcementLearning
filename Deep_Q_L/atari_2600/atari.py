...
class AtariDeepqNet:
    ...
    def populate_memory(self, episodes=64):
        """
    ...
    def _learn(self, sess, episode, batch_size):
        ...
        mini_batches.update(
            {'targets': target_mini_batch}
        )

        loss, acc = self.find_loss(sess, mini_batches)
        self.summarize(sess, mini_batches, episode)
    ...
    def get_q_values(self, sess, m_batches):
        """
            ...
            - Target Q values
        """
        rewards = m_batches.get('rewards')
        next_states = m_batches.get('next_states')
        dones = m_batches.get('dones')
        batch_len = m_batches.get('batch_len')
        target_Qs_batch = []
    ...
    def summarize(self, sess, mini_batches, episode):
        """
            Writes tf summaries
        """
        summary = sess.run(
    ...
    def _save(self, sess, count):
        ...
        """
        if not count % 5:
            path = self.saver.save(sess, '.models/atari.ckpt')
    ...
    def play(self, episodes=25):
        ...
        """
        with tf.compat.v1.Session() as sess:
            self.saver.restore(sess, '.models/atari.ckpt')
            final_reward = []
            ...
            for episode in range(episodes):
                ...
                while True:
                    Q_s = sess.run(self.output,
                                   feed_dict={self.inputs: state.reshape(1, *self.state_size)})
                    choice = np.argmax(Q_s)
                    action = self.actions_choice[choice]
                    next_state, reward, done, _ = self.env.step(action)
                    self.env.render()
                    ...
                    next_state, stacked_frames = stack_frames(
                        next_state, stacked_frames)
                    state = next_state
            print('Score: ', np.sum(final_reward) / episodes)
            self.env.close()
...
def main():
    ...
    clf = AtariDeepqNet()
    clf.populate_memory()
    clf.train(episodes=50, batch_size=64, max_steps=1000)
    clf.play()
